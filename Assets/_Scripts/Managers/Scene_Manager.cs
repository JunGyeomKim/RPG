...
public class Scene_Manager : MonoSingleton<Scene_Manager> {
    ...
    public void LoadScene(eSceneType _type, bool _retry = false, bool _async = true)
    {
		if (CurrentState == _type && _retry == false)
			return;

		if(_retry)
		{
			CurrentState = eSceneType.SCENE_LOBBY;
		}
	...
	void Update ()
	{
        ...
        if (CurrentState == eSceneType.SCENE_NONE)//여기 아래로 안들어옴.
            return;


		//if(NextState != eSceneType.SCENE_NONE && CurrentState != NextState) ///현재 씬과 다음씬이 같으면 리턴.
            ...
            case eSceneType.SCENE_GAME:
                {
                    ...
                    InGameUIManager.Instance.GameUIInit();
                }
                break;
    ...
    void DisableScene(eSceneType _type)
    {
        ...
        switch (_type)
        {
            ...
            case eSceneType.SCENE_GAME:
                ...
                InGameUIManager.Instance.DisableUI();
                break;
            case eSceneType.SCENE_LOBBY:
