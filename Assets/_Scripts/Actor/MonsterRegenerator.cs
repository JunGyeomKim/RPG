...
public class MonsterRegenerator : BaseObject
{
    ...
    // Once Event
    public MonsterRegenerator PreGenerator = null;
	...
	{
		MonsterPrefab = 
			ActorManager.Instance.GetMonsterPrefab(MonsterType);
			//Resources.Load(
			//	"Prefabs/" + MonsterType.ToString())
			//	as GameObject;
		...
		switch (RegenType)
		{
			...
			case eRegeneratorType.TRIGGER_EVENT:
                ...
                break;
            case eRegeneratorType.ONCE_EVENT:
                {
                }
				break;
		}
            ...
            case eRegeneratorType.ONCE_EVENT:
                {
                    if(IsGenerated == false)
                    {
                        ...
                        else if (PreGenerator.FINISH == true)
                        {
                            RegenMonster();
                        }
                    }
                    else if (ListAttachMonster.Count == 0)
                        IsFinish = true;
                }
                break;
		}
	...
	void RegenMonster()
	{
        ...
        for (int i = ListAttachMonster.Count;
			i < MaxObjectNum; i++)
		{
			Actor actor = ActorManager.Instance
				.InstantiateOnce(
				MonsterPrefab,
			...
			case eRegeneratorType.TRIGGER_EVENT:
				{
                    ...
                    //if (actor != null && actor.gameObject.name.Contains("Player")) //밑이랑 같은 뜻
                    if (actor != null && actor.IS_PLAYER == true)
					{
						RegenMonster();
					}
				}
